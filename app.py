...
download_status: Dict[str, Dict[str, object]] = {}
...
def _update_status(task_id: str, payload: Dict[str, object]) -> None:
    with status_lock:
        previous = download_status.get(task_id, {})
        merged = previous.copy()
        merged.update(payload)
        download_status[task_id] = merged


def download_m3u8(url: str, filename: str, task_id: str) -> None:
    try:
        ...
        _update_status(
            task_id,
            {
                "message": f"找到 {total_segments} 个视频片段，开始下载...",
            },
        )
        ...
        with open(temp_output_path, "wb") as output_file:
            ...
            def abort_download(message: str) -> None:
            ...
            for index, segment in enumerate(playlist.segments):
                ...
                try:
                    ...
                    abort_download(
                        f"片段 {index + 1} 下载失败: {exc}。已下载的片段: {index}/{total_segments}"
                    )
                    return
                ...
                if segment.key and segment.key.method and segment.key.method.upper() != "NONE":
                    ...
                    if method != "AES-128":
                        abort_download(f"片段 {index + 1} 使用了不支持的加密方式: {method}")
                        return
                    ...
                    if key_bytes is None:
                        try:
                            ...
                            abort_download(f"片段 {index + 1} 密钥下载失败: {key_error}")
                            return

                    iv_hex = segment.key.iv
                    if iv_hex:
                        iv_str = re.sub("^0x", "", iv_hex.lower()).zfill(32)
                        try:
                            iv_bytes = bytes.fromhex(iv_str)
                        except ValueError as exc:
                            abort_download(f"片段 {index + 1} IV 解析失败: {exc}")
                            return
                    else:
                    ...
                    if len(iv_bytes) != 16:
                        abort_download(f"片段 {index + 1} IV 长度无效，无法解密")
                        return
                    ...
                    try:
                        ...
                        abort_download(f"片段 {index + 1} 解密失败: {exc}")
                        return
                ...
                average_segment_size = downloaded_bytes / (index + 1)
                ...
                _update_status(
                    task_id,
                    {
                        ...
                        "message": f"下载中... {index + 1}/{total_segments} ({progress}%)",
                    },
                )
...
def start_download():
    ...
    worker = threading.Thread(target=download_m3u8, args=(url, filename, task_id), daemon=True)
    worker.start()
